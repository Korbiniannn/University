...
def find_center(polygons):
    ...
    for polygon in polygons:
        lat, long, area = find_centroid(polygon)
        average_lat += lat * area
        average_long += long * area
        average_area += area

    centered_lat = average_lat / average_area
    centered_long = average_long / average_area
...
def find_closest_state(tweet, state_centers):
    ...
    """
    "*** YOUR CODE HERE ***"
    distance_count = float('inf')
    state_count = 0
    location = tweet_location(tweet)
    for key, value in state_centers.items():
        if geo_distance(location, value) < distance_count:
            distance_count = geo_distance(location, value)
            state_count = key
    return state_count
...
def group_tweets_by_state(tweets):
    ...
    "*** YOUR CODE HERE ***"
    state_center = {}
    for name, coords in us_states.items():
        state_center[name] = find_center(coords)
    for state, _ in us_states.items():
        tweets_by_state[state] = []
    for tweet in tweets:
        closest_state = find_closest_state(tweet, state_center)
...
def most_talkative_state(term):
    ...
    for tweet in tweets:
        closest_state = find_closest_state(tweet, state_center)
        state_counter[closest_state] += 1
    for name, value in state_counter:
        if value > count:
            count = value
            count_name = name
    return count_name
...
def average_sentiments(tweets_by_state):
    ...
    sentiment_with_state = defaultdict(int)
    tweet_counter = defaultdict(int)
    for state_name, tweets in tweets_by_state.items():
        for tweet in tweets:
            sentiment = analyze_tweet_sentiment(tweet)
            if sentiment != None:
                tweet_counter[state_name] += 1
                sentiment_with_state[state_name] += analyze_tweet_sentiment(tweet)
    for key, value in sentiment_with_state.items():
        averaged_state_sentiments[key] = value / tweet_counter[key]
...
def group_tweets_by_hour(tweets):
    ...
    """
    tweets_by_hour = {hour: [] for hour in range(24)}

    for tweet in tweets:
        tweet_tm = tweet_time(tweet)
        tweet_hour = tweet_tm.hour
